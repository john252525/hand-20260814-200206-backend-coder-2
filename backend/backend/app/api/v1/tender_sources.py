...
class SourceCreate(BaseModel):
    ...
    def validate_type(cls, v):
        if v not in ALLOWED_TYPES:
            raise ValueError('type must be one of aggregator_api, direct_api')
        return v
...
async def update_source_full(source_id: uuid.UUID, payload: SourceCreate, db: AsyncSession = Depends(get_db)):
    ...
    source.api_url = payload.api_url
    source.api_key_encrypted = payload.api_key or source.api_key_encrypted
