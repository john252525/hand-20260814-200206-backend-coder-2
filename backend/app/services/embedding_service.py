...
async def get_embedding_or_empty(text: str) -> List[float]:
    """Обёртка для обратной совместимости: возвращает только вектор.
    Флаг is_degraded игнорируется или может быть сохранён отдельно.
    """
    emb, _ = await generate_embedding(text)
    return emb
