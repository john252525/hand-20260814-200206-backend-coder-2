...
from app.models import Tender, TenderStatusHistory, LotSupplier, Supplier, Task, TenderPosition, TenderDocument, Communication
...
async def list_tenders(
    ...
    query = select(Tender).options(
        ...
        selectinload(Tender.lot_suppliers),
...
async def create_tender(payload: TenderCreate, db: AsyncSession = Depends(get_db)):
    tender = Tender(**payload.model_dump(exclude={"documents_urls", "skip_auto_processing"}))
    db.add(tender)
    await db.flush()
...
async def get_tender(tender_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "data": {
            ...
            "matched_categories": [{"id": tender.matched_category_id, "name": tender.category.name if tender.category else None, "similarity": tender.similarity_score}] if tender.matched_category_id else [],
            "score": tender.score,
...
async def tender_communications(tender_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not tender:
        raise HTTPException(status_code=404, detail={"code": "NOT_FOUND", "message": "Tender not found"})
    # Загружаем все коммуникации одним запросом
